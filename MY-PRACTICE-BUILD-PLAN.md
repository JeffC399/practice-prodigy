...
| 2026-08-24 | **Slice H prep (H.2 + H.3): ship-ready.** Phase 162. Release-notes entry added ("My Practice is live — α launch") so returning users see the announcement in the "What's new" modal. Module-registry description sharpened. Everything on the code side is ready; remaining Slice H work is operational — apply migration 0009 to prod, cut a `pre-my-practice` git tag, set `NEXT_PUBLIC_MY_PRACTICE_ENABLED=true` in Vercel, redeploy. The registry already auto-promotes my-practice from `bucket:"next"` to `bucket:"now"` when the flag flips, so no additional code change at flip time. Slices A → G partial + I complete. Original 8-slice plan reached alpha ship-ready state ~2 days of active work vs the plan's 4–6 months estimate. |
| 2026-08-24 | **Slice G partial (G.1, G.4, G.6, G.7).** 3-step onboarding wizard on first visit (welcome / try starter / what's next; skippable at every step; auto-creates a real routine from picked template). Targeted a11y pass: `role="log"` + `aria-live="polite"` on the Coach message log, Esc-to-close on the onboarding modal, `role="status"` on the auto-suggest banner. Docs synced. **Deferred**: G.2 progressive-enrichment prompts (Profile tab dependency), G.5 mobile responsive audit (real device testing), G.3 empty states (already reasonable across shipped tabs; final polish deferred). Copy polish (G.6) folded into each phase inline. |
| 2026-08-24 | **Slice F Active mode shipped — full β milestone.** Phase 157 combined F.10 + F.11: tool-call subsystem + proposal-card confirmation UX in one shipment (inseparable). Three additive tools (propose_song / propose_collection / propose_routine) with Zod schemas + AI SDK v7 human-in-the-loop dispatch. Client dispatches the mutation locally after user confirms; addToolResult feeds outcome back into the conversation. Agency-mode picker in Settings enabled — user opts in. Deferred as follow-ups: propose_drill (module configs), edit_* mutations, delete_* (need versioning). Slice F reached full β per §12 gate. Next: Slice G (Onboarding + Polish) or Slice H (Ship) per the plan. |
| 2026-08-22–23 | **Slice F Passive mode shipped (partial β).** Phases 139–147 + 155–156. Hybrid auth (user picked "Gateway default + BYOK escape hatch"): F.1 AI SDK v7 + config store + /api/ai/chat streaming route with model allow-list + payload caps. F.4 ChatView on /my-practice/coach with streaming + shortcut prompts. F.3 Passive system prompt (voice + anti-hallucination guardrails + routine-draft format spec) + context-assembly (routines / songs / drills-per-module / sheets / levels / practice pulse / streak). F.5+F.6 routine-draft parser + inline draft card + Why-this panel + Save-as-routine (source="ai-coach"). F.7+F.8 AI-suggest methodology per-item ("?" button) + bulk builder button, `/api/ai/suggest-methodology` with generateObject + Zod schema. F.9 persisted conversation history (30-cap) + strip-nav for switching. F.13 Settings tab (6th) with model picker + BYOK form (masked + remove) + agency-mode picker (Active shown as coming-soon). F.12 usage indicator (token counts per conversation + header chip). **F.10 + F.11 (Active mode tool calls + confirmation flow) intentionally deferred** — the tool-call subsystem + per-mutation confirmation UX is a meaty piece of code that shouldn't ship autonomously without testing feedback. Full β milestone lands when Active mode does. Passive mode is enough for α testing today. |
| 2026-08-23 | **Slice I shipped — α.4 mini-milestone.** New slice, outside the original 8-slice plan. Prompted by the user's screenshot showing a Key Sequencer library with a fretboard-drill name pattern that begged for grouping. 7 phases (148–154). I.1 data model + `useCollections` Zustand store + 10th sync adapter + migration 0009. I.2 Collections tab in My Practice with create/edit/delete modal (name + emoji + 10-swatch color + description). I.3 membership editor: `CollectionsMembershipPicker` popover + `CollectionsChip` inline on every user library card (Songs, KS, Scale, Bass user drills, Sheets — shipped Bass drills excluded). I.4 `CollectionsSectionedList` collapsible per-collection headers wired into all 5 module library views + zero-chrome short-circuit when the user has no collections. I.5 auto-suggest banner: name-prefix heuristic (trim at first "(" or " - ", 10-char min, 2+ item threshold) with suppression guards (existing collection name, already-shared collection, dismissed key). I.6 "Run collection" button converts members to RoutineItems + saves as routine (source="template", sourceRef=collection.id) + jumps to builder. I.7 docs sync. Cross-module + many-to-many + flat (no nesting) as designed with the user. Migration 0009 applied to `practice-prodigy-dev`; prod pending. Real-estate win lands the moment a user creates their first collection. |
| 2026-08-21 | **Slice E shipped — α.3 mini-milestone.** Phases 135–138. E.4 (author articles) + E.5 (author templates) were already done ahead of the code (8 markdown articles + 8 templates under `src/content/`). E.1 build-time codegen (`scripts/generate-methodology-content.mjs`) emits a typed `.generated.ts` inlining articles + templates — no runtime YAML / markdown-parser dep. E.2 + E.3 shipped together: MethodologyTab list view (2-col cards showing scope / summary / category chips / template count) + article view + custom ~180-LOC markdown renderer scoped to the constructs the shipping articles actually use. E.6 "Try this template" → saveRoutine with `source: "template"` + `sourceRef: template.id` + methodologyId preserved + template items converted to `RoutineItem` union with fresh ids; navigates to the builder. E.7 living-doc sync. **The Methodology tab is no longer a coming-soon placeholder** — 4 of 5 tabs now live (Routines / Songs / Reports / Methodology); only Profile remains. ~1 day actual vs the plan's 2-week estimate (content was pre-authored, cutting E.4+E.5's 8d down to zero). Next: Slice F (AI Coach — biggest remaining, gates β) or Slice G (Onboarding + Polish). |
| 2026-08-21 | **Slice C shipped + D.6 unblocked — α.2 mini-milestone.** Phases 128–134. C.1 Song type + useSongsLibrary Zustand store + 9th sync adapter (no schema migration — `songs` table already in 0007). C.2 Songs tab list view with SongCard (inline-editable title, click-to-cycle status, sheet-link chip, hover Edit/Delete), status/sort filters, case-insensitive search. C.3 SongFormModal covering the full metadata surface (artist / key / time-sig / genre / status buttons / target date / linked sheet / notes) with Save-writes-once semantics. C.4 song routine item end-to-end — composer + picker registration + takeover-route (linked → sheet playback, unlinked → SongInlineBody in player) + per-song practice attribution in routine-executor.advance/previous (skipped items don't get credit). C.5 / D.6 (combined) SongsProgressPanel in Reports with total time + days-since-last + "stale" nudge at 14 days. C.6 SongImportFromSheets bulk importer with dupe-filter (already-linked sheets hidden) and metadata prefill. C.7 living-doc sync. ~1 week actual vs the plan's 2-week estimate. Next: Slice E (Methodology library — the article-per-methodology surface Slice B.13 picker already references) or Slice F (AI Coach — biggest remaining, gates β). |
| 2026-08-21 | **Slice D shipped (except D.6 Songs — blocked on Slice C) — α.1 mini-milestone.** Phases 118–127. D.1 pure aggregation helpers (range filter + secondsByDay / -category / -module / -itemId / -methodology + streak). D.2 tab layout + range picker + 4 headline stats. D.3 category time bar (reuses Slice B.8's CategoryTimeBar). D.4 custom-SVG 12-week calendar heatmap with primary-color opacity intensity buckets. D.5 daily trend bars + 7-day rolling-average polyline (custom SVG). D.7 levels & progression panel with inline level-picker popover (doubles as the level editor until Slice E/F's Profile tab lands). D.8 methodology mix chip that resolves session items → routine items → methodology, with an "Unattributed" slice for ad-hoc time. D.9 global header PracticeChip linking to Reports. D.10 CSV export (Blob download) + PDF via window.print() with a `.reports-print-mode` visibility-toggle print CSS. Zero chart-lib deps — every viz is custom SVG. D.6 Songs progress panel deferred pending Slice C. ~1 week actual vs the plan's 2-week estimate. Next: Slice C (unblocks D.6) or Slice F (AI Coach, gates β). |
| 2026-08-21 | **Slice B shipped — α (alpha) milestone reached.** All 16 sub-slices (B.1 → B.16) landed across phases 101–117: routine data model + Zustand store + sync adapter, 5-tab shell, routine cards + inline edit, dedicated builder with dnd-kit drag-reorder, all 7 item composers (drill / key-drill / scale-drill / leadsheet / metronome / custom / rest), full-screen player + take-over into all four drilling modules via `?routineMode=1`, resume-if-interrupted, end-of-routine per-item breakdown + category-time bar + vibe-check (B.12), per-item methodology picker (B.13), routine-level methodology (B.13.5), Routine Overview pre-run gate (B.14), rest auto-advance + WebAudio chimes + M-mute shortcut + settings surface (B.15), living-doc updates (B.16). Only bug: KS setup config was resetting on return from a launched drill (Phase 97 — fix already committed). Total build time: ~4 weeks against a 6-week estimate. Next: Slice C (Songs library) or Slice D (Reports); AI Coach (Slice F) is the biggest remaining slice and gates β. |
