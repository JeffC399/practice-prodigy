...
Total remaining: ~5-8 sessions to complete the LSB Basic Tier as a Dorico-class authoring tool. Phases may re-prioritize based on tester feedback once the build is in user hands.
...
- **Remaining Basic Tier work** (see §8.5 for the phase table): 30 (MIDI input), 31 (selection model), 32 (pickup + bars-per-line), 33 (share via URL), 34 (print polish), 35 (library polish). ~5-8 sessions.
