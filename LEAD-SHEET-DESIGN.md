...
|---|---|---|---|
| 29.1 | **Slurs.** VexFlow `Curve` (curved phrase arc, distinct from tie). Editor: multi-select notes then group into a slur (like tupletGroup — a new `slurGroup?: string` field on MelodyNote). Cross-line slur handling follows the same pattern as cross-line ties. | 1 session | The other half of the original Phase 29 scope. Split off so 29 could ship cleanly. |
...
Total remaining: ~6-9 sessions to complete the LSB Basic Tier as a Dorico-class authoring tool. Phases may re-prioritize based on tester feedback once the build is in user hands.
...
- **Remaining Basic Tier work** (see §8.5 for the phase table): 29.1 (slurs), 30 (MIDI input), 31 (selection model), 32 (pickup + bars-per-line), 33 (share via URL), 34 (print polish), 35 (library polish). ~6-9 sessions.
