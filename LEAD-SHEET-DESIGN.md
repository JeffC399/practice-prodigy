...
| 32 | 2026-07-01 | **Bars-per-line toggle.** `Sheet` gains optional `measuresPerLine?: 4 \| 6 \| 8`. `SheetSurface` already accepted the prop with a default of 4; both the editor and view/print pages now pass `sheet.measuresPerLine`. Editor toolbar gains a compact `Bars/line 4 / 6 / 8` chip triplet next to the zoom controls; clicking a value writes through `updateMeta` so it persists on the sheet. Print output respects the choice. **Pickup (anacrusis) measure deferred** to a follow-up — audio playback engine needs a per-measure beat-length rewrite for correct timing, which is a substantial refactor. |
| 33 | 2026-07-01 | **Share via URL + JSON download.** New pure module `src/lib/sheets/share.ts` — `encodeSheet(sheet)` returns a base64url-safe JSON string; `decodeSheet(encoded)` is the inverse (returns `null` on any parse failure with a basic shape guard). `buildShareUrl` composes the full share URL and flags payloads over a 20 KB soft cap. New `ShareModal` component: two options — "Copy link" (encoded `?d=` URL, copies to clipboard, briefly shows "Copied") and "Download JSON" (raw sheet as a `.json` file for cases too big for a URL or where a file transfer is preferred). New route `/sheets/shared?d=<encoded>` decodes the payload, renders the sheet read-only via the standard `SheetSurface`, and offers a "Save to my library" CTA. Library store gains an `importSheet(incoming)` action that clones the imported sheet with a fresh id + timestamps and appends to the local library. Editor header gains a "Share" button next to `?`. `useSearchParams` wrapped in `<Suspense>` so `/sheets/shared` prerenders cleanly under Next.js 16. |
| 35 | 2026-07-01 | **Library polish — search + JSON import + refreshed copy.** `/sheets` page gains a search input (`Search` icon prefix, live filter as you type) that matches against title / composer / lyricist / arranger / style / source / key. Filtered count shows "N of M" when a search is active; empty search results get their own "No sheets match ‘X’" empty state with a Clear button. New "Import" button next to search opens a hidden `<input type="file">` — user picks a JSON file (from the Phase 33 share flow) → parsed, validated, cloned via `importSheet`, then editor opens on the imported copy. Also accepts a base64url encoded string (someone pasted a share-URL payload into a text file). Library header description refreshed — LSB is now feature-complete for melody + lyrics + form markings + live audio playback + MIDI input, no longer just chord charts. **Thumbnails + mobile tab-strip + full a11y pass** deferred to a follow-up. |
...
Total remaining: **LSB Basic Tier essentially done.** Everything from the original 8.5 roadmap is shipped. Small polish items still open — auto-detect ottava suggestions (shipped as 31.7), pickup measures (deferred to when we do the audio-engine refactor), live-rendered thumbnails on library cards, mobile-touch tab-strip, full a11y pass. Phases may re-prioritize based on tester feedback once the build is in user hands.
...
- **Basic Tier essentially complete.** Everything from the original §8.5 roadmap (through Phase 35) is shipped. Deferred to follow-ups: pickup measure (needs audio-engine refactor), print polish (page numbers / footer / page breaks), thumbnails + mobile tab-strip on the library page.
- **Advanced Tier**: stays in `bucket: "later"` / `status: "sketch"` until basic ships and is in production use.
| 2026-07-11 | **Phase 36 — LSB library thumbnails shipped.** LSB Basic Tier polish item A from the audit. `/sheets` cards now render a live 240×120 preview of the first line of music via a new `SheetThumbnail` component that scales the full `SheetSurface` down with CSS transform. Full details in PROJECT-DESIGN.md Phase 36 entry. Basic Tier polish sequence: A (thumbnails) done, next is B (print polish — page numbers, copyright footer, page-break controls). |
| 2026-07-11 | **Phase 37 — LSB print polish shipped.** Basic Tier polish item B: page numbers (`Page N of M`) in the bottom-right + optional copyright text in the bottom-left of every printed page + `break-inside: avoid` on the sheet-paper wrapper. Standard professional-chart footer via `@page` margin boxes. Full details in PROJECT-DESIGN.md Phase 37 entry. Item C next (mobile / touch tab-strip editor layout). |
| 2026-07-11 | **Phase 38 — LSB mobile responsiveness pass shipped.** Basic Tier polish item C: pragmatic mobile-friendly padding + toolbar wrapping + horizontal-scroll preview on both view and edit pages. Desktop layout unchanged. Full tab-strip refactor (design doc §3 spec) deferred as a follow-up. See PROJECT-DESIGN.md Phase 38 entry for scope-choice reasoning. Item D (a11y pass) next. |
| 2026-07-11 | **Phase 39 — LSB a11y pass shipped.** Basic Tier polish item D: skip link + global keyboard focus rings + id="main-content" on every route + module-switcher aria-label. Applies app-wide, not just LSB. Basic Tier is now essentially closed — the deferred item E (pickup measure) is a distinct audio-engine refactor rather than "polish". Full details in PROJECT-DESIGN.md Phase 39 entry. |
| 2026-07-11 | **Phase 40 — Pickup measure shipped. LSB Basic Tier is now genuinely complete.** Every item from the original v2 design pass (§2 Basic Tier scope, §8.5 phased roadmap) is now live: chord charts, melody with full rhythm vocabulary, lyrics with hyphen + underscore, form markings + section labels, print with page numbers + copyright, share via URL / JSON, library with search + thumbnails + import, MIDI input, live audio playback with instrument picker + mixer, cross-measure ties + slurs + ottava marks, undo/redo, mobile responsiveness, a11y baseline, and now pickup measures. Advanced Tier (mid-piece key/time changes, multi-voice, MusicXML, multi-page layout with manual page breaks, real-time collab, audio playback of the full chart with comping) stays deferred as originally planned. See PROJECT-DESIGN.md Phase 40 entry for the pickup implementation approach — sheet-level `pickup?: number` + `getMeasureBeats(sheet, mi)` helper consumed by both renderer + audio engine. |
