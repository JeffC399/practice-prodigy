...
| 2026-06-28 | Persistent header module switcher | New → v1.1 | Always-visible top bar across every Practice Prodigy screen with the current module's name on the left and a small "Switch module" dropdown (or icon-tab strip) listing every module — implemented ones link, future ones show a muted "Coming soon" pill. Surface for the 9-item platform vision so the user senses scope from day one without crowding the active surface. Pairs with the persistent footer below. |
| 2026-06-28 | Persistent footer with credits + roadmap link | New → v1.1 | Slim footer band on every screen: left-side small "Practice Prodigy · v0.X" build label, right-side links to `/roadmap`, GitHub/feedback, and a one-line build credit. Anchors the app as a serious product (vs an unsigned web tool) and gives the user a discoverable jump-off to the roadmap surface. |
| 2026-06-28 | `/roadmap` page surfacing the 9-item vision | New → v1.1 | Standalone marketing/transparency surface that mirrors PROJECT-DESIGN.md §9 publicly: timeline columns (Now / Next / Later) with each of the 9 modules tagged by status (Live / In build / Designed / Sketch). Bonus: each card links to the GitHub issue tracking it. Doubles as a recruiting / credibility artifact for early users and potential collaborators. |
